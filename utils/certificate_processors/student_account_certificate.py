...
class StudentAccountCertificateProcessor:
    ...
    def process_data(self, data):
        ...
        # 先确定操作类型
        operation_type = "充值"  # 默认为充值
        biz_type = processed_data.get('sBizType', '')
        pay_info = processed_data.get('sPay', '')
        
        # 判断是充值还是提现
        if '提现' in biz_type or '提现' in pay_info or 'Title' in processed_data and '提现' in processed_data['Title']:
            operation_type = "提现"
        elif '充值' in biz_type or '充值' in pay_info or 'Title' in processed_data and '充值' in processed_data['Title']:
            operation_type = "充值"
        ...
        if 'sPay' in processed_data:
            ...
            if '充值金额' in pay_info or '提现金额' in pay_info:
                ...
                if operation_type == "充值":
                    processed_data['sPay'] = f"充值金额：{formatted_amount}"
                elif operation_type == "提现":
                    processed_data['sPay'] = f"提现金额：{formatted_amount}"
        
        # 处理余额信息 - 关键修改：在余额前面标注充值金额或提现金额
        if 'dSumBalance' in processed_data:
            ...
            if 'sPay' in processed_data:
                ...
                if amount_match:
                    ...
                    try:
                        ...
                        operation_amount = f"¥{amount_value:,.2f}"
                    except:
                        operation_amount = pay_info
            ...
            if operation_amount:
                if operation_type == "充值":
                    processed_data['sOperationAmount'] = f"充值金额：{operation_amount}"
                elif operation_type == "提现":
                    processed_data['sOperationAmount'] = f"提现金额：{operation_amount}"
            else:
                # 如果没有具体金额，至少标注操作类型
                processed_data['sOperationAmount'] = f"{operation_type}操作"
        ...
        if operation_type == "充值":
            ...
            processed_data['sProofName'] = '充值凭证'
        elif operation_type == "提现":
            processed_data['Title'] = '提现凭证' 
            processed_data['sProofName'] = '提现凭证'
        else:
            # 默认情况，如果无法判断类型
            processed_data['Title'] = '充值提现凭证'
            processed_data['sProofName'] = '充值提现凭证'
